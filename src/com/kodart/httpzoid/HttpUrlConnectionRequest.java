...
public class HttpUrlConnectionRequest implements HttpRequest {
	...
	@Override
	public boolean send() {
		final boolean hasRequestBeenSuccesful[] = {false};
		...
		if (network.isOffline()) {
			...
			try {
				...
				sendData(connection);
				final HttpDataResponse response = readData(connection);
				resultedAction = new Action() {
					@Override
					public void call() {
						if (handler != null) {
							if (response.getCode() < 400) {
								hasRequestBeenSuccesful[0] = true;
								handler.success(response.getData(), response);
							} else {
								handler.error((String) response.getData(), response);
							}
				...
				Log.e(TAG, e.getMessage());
				resultedAction = new NetworkFailureAction(handler, e.getNetworkError());
			} catch (SocketTimeoutException e) {
				Log.e(TAG, e.getMessage());
				resultedAction = new NetworkFailureAction(handler, NetworkError.Timeout);
			} catch (ProtocolException e) {
				Log.wtf(TAG, e.getMessage());
				resultedAction = new NetworkFailureAction(handler, NetworkError.UnsupportedMethod);
			} catch (Throwable e) {
				Log.wtf(TAG, e);
				resultedAction = new NetworkFailureAction(handler, NetworkError.Unknown);
			} finally {
				if (connection != null)
					connection.disconnect();
			}
		...
		return hasRequestBeenSuccesful[0];
	}
}